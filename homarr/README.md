...
公式イメージ内蔵のmock DownloadsデータにはUbuntu ISO以外のtorrentも含まれ、Composeや環境変数から内容を差し替えられません。このリポジトリでは公式イメージを改造せずに維持するため、初期ボードからDownloadsウィジェットだけを非表示にしています。実際のqBittorrentなどを連携した後は、GUIからDownloadsウィジェットを追加できます。
